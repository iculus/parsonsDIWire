...
if (!preview) { //draws exact shape from file bendMe.svg
    ...
    stroke(0,0,255);  //needs any value to draw preview stroke - ???
    noFill();
    shp.draw();
    
    //eventually update position and angle to preview entire geometry
    //center onject around height/2,width/2
    ...
    if (points != null) {
      fill(255);  //fills shape white
      stroke(0, 200, 0);  //RGB of outline line on shape 
      strokeWeight (5);
      
      
      //makes points on shape   
      beginShape();
...
void calcs () {
  float lastAng = 0;  //was float (lastAng=0).  changed to get rid of warning in first angle
  float lastlastAng = 0;
...
void sendArd () { //sends bend angles and feed lengths to the arduino board for printing
  ...
  for (int i = 2; i < steps; i++) {
    ...
    // this said - for (int i = 2; i < steps+233; i++) {
